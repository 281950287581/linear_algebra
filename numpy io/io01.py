...
file：要保存的文件，扩展名为 .npy，如果文件路径末尾没有扩展名 .npy，
该扩展名会被自动加上
arr:要保存的数组
allow_pickle: 可选，布尔值，允许使用 Python pickles 
保存对象数组，
...
如果文件路径末尾没有扩展名 .npz，该扩展名会被自动加上
args: 要保存的数组，可以使用关键字参数为数组起一个名字
...
对应的使用 loadtxt() 函数来获取数据
np.loadtxt(FILENAME, dtype=int, delimiter=' ')
np.savetxt(FILENAME, a, fmt="%d", delimiter=",")
参数 delimiter 可以指定各种分隔符、
针对特定列的转换器函数、需要跳过的行数等
"""
# a = np.array([1,2,3,4,5])
